  ...
  for(i = 0; i < numFinal; i++)
    {
      ...
      ASSERT_EQUALS_RECORDS(fromTestRecord(schema, finalR[i]), r, schema, "compare both records");
    }
...
void testPrimaryKeyImpl(void)
{
  ...
  int numInserts = 10, numUpdates = 2, numFinal = 2, i;
  Record *r;
  RID *rids;
  Schema *schema;

  testName = "Testing primary key";
  schema = testSchema();
  rids = (RID *) malloc(sizeof(RID) * numInserts);
